...
async def on_startup(dp):
    ...
    msk_time = get_moscow_time().strftime("%H:%M")
    logger.info(f"🚀 v9.5 LIVE — {CAFE_NAME} | MSK: {msk_time} | "
...
async def healthcheck(request):
    return web.Response(text="CafeBotify v9.5 LIVE ✅", status=200)

# ========================================
if __name__ == '__main__':
    logger.info(f"🎬 v9.5 START — {CAFE_NAME} | PORT: {WEBAPP_PORT}")
    
    # ✅ КРИТИЧЕСКИ ВАЖНО: AIOHTTP APP ПЕРЕД executor!
    app = web.Application()
